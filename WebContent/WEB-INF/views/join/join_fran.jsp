...
$(document).ready(function(){
	...
	//--------필수정보입력---------
		...
		if( $("#franName").val() == "" ){
			$("#franNamev").text( "프랜차이즈를 선택하세요." ); 
			$("#franNamev").css( "color", 'red' ); 
			return false;
		}
	//-------------------------------
	//아이디중복
	...
	$("#btnKey").click(function(){
		...
		sendRequest("POST", "/frankeyverify", params, callbackKey);
	})
	
	function callbackKey(){
		console.log("Key 콜백함수 호출");
	...
	function keyCheckResult() {
		
		var keyCheckResult = JSON.parse(httpRequest.responseText);
		console.log(keyCheckResult);
		
		checkkey = keyCheckResult.result
...
<form action="/franjoin" method="POST">
